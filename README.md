...
| `favicon.svg` / `favicon.ico` / `apple-touch-icon.png` | ファビコン（白背景 + 黒の ρ） |
| `tools/make_favicon.py` | ファビコンの生成スクリプト |

## ファビコン

白背景に黒のギリシャ文字「ρ」。DejaVu Sans Bold のアウトラインから生成しています。
...
背景を白の不透明にしてあるため、ダークテーマのタブバーでも iOS のホーム画面でも
そのまま判読できます（透明背景だと黒い ρ が背景に沈む、
iOS が透明部分を黒で塗り潰す、という 2 つの問題があった）。
